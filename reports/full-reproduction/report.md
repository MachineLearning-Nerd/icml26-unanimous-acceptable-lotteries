...
Raw CSV/JSON, claim contracts, source audits, exact commands, seeds, independent checker outputs, controls, and proof certificates are mirrored into the candidate logbook. The pinned environment is Python 3.12 with a dependency-free `uv.lock`; the implementation uses the standard library only.
