...
def run_claim3_exhaustive(Agent, learn_hyperplane, satisfies, simplex_grid, root: Path):
    ...
    (artifact_dir / "exhaustive_checker_output.json").write_text(json.dumps(checker, indent=2, sort_keys=True) + "\n")
    (artifact_dir / "exhaustive_negative_control_output.json").write_text(json.dumps(controls, indent=2, sort_keys=True) + "\n")
    (artifact_dir / "exhaustive_proof_certificate.json").write_text(json.dumps(proof, indent=2, sort_keys=True) + "\n")
